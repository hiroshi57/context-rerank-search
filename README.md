...
- **DB**: `service/db.py`（SQLite）。文書コーパス保存、全クエリ tenant_id 強制フィルタ＝**テナント分離**
- **API**: `service/api.py`（FastAPI）。docs(投入) / search(ハイブリッド/リランク) / report(HTML結果)
- **HTMLレポート**: `service/report_html.py`（検索結果＋根拠＋抜粋、XSSエスケープ）
- **フロント**: `frontend/`（React+Vite）。**検索UI**＋**精度評価ビュー**の2画面。ビルド不要は `frontend/standalone.html`
